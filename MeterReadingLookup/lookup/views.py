...
from .models import RegisterReadings
...
def upload_file(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render(request, 'lookup/upload_success.html',{'file_name': file_name,
                                                                 'file_content': decoded_file})
    else:
        form = UploadFileForm()
...
def flowfile_detail(request,file_name):
    ...
    if MPAN_query:
        ...
        file_readings = file_readings.filter(mpan__serial_number__startswith=MPAN_query)
